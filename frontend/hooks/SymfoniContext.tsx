...
import { HabitSource } from "./../utils/HabitSource";
import { HabitSource__factory } from "./../utils/factories/HabitSource__factory";
import { HabitTracker } from "./../utils/HabitTracker";
import { HabitTracker__factory } from "./../utils/factories/HabitTracker__factory";
...
export const ProviderContext = React.createContext<[providers.Provider | undefined, React.Dispatch<React.SetStateAction<providers.Provider | undefined>>]>([
    defaultProvider,
    () => {},
]);
...
export const HabitSourceContext = React.createContext<SymfoniHabitSource>(emptyContract);
export const HabitTrackerContext = React.createContext<SymfoniHabitTracker>(emptyContract);
...
export interface SymfoniHabitSource {
    instance?: HabitSource;
    factory?: HabitSource__factory;
}

export interface SymfoniHabitTracker {
    instance?: HabitTracker;
    factory?: HabitTracker__factory;
}
...
export const Symfoni: React.FC<SymfoniProps> = ({ showLoading = true, autoInit = true, ...props }) => {
    ...
    const [HabitSource, setHabitSource] = useState<SymfoniHabitSource>(emptyContract);
    const [HabitTracker, setHabitTracker] = useState<SymfoniHabitTracker>(emptyContract);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        const doAsync = async () => {
            ...
            const finishWithContracts = (text: string) => {
                setHabitSource(getHabitSource(_provider, _signer));
                setHabitTracker(getHabitTracker(_provider, _signer));
                finish(text);
            };
    ...
    const getHabitSource = (_provider: providers.Provider, _signer?: Signer) => {
        let instance = _signer
            ? HabitSource__factory.connect(ethers.constants.AddressZero, _signer)
            : HabitSource__factory.connect(ethers.constants.AddressZero, _provider);
        const contract: SymfoniHabitSource = {
            instance: instance,
            factory: _signer ? new HabitSource__factory(_signer) : undefined,
        };
        return contract;
    };
    const getHabitTracker = (_provider: providers.Provider, _signer?: Signer) => {
        let instance = _signer
            ? HabitTracker__factory.connect(ethers.constants.AddressZero, _signer)
            : HabitTracker__factory.connect(ethers.constants.AddressZero, _provider);
        const contract: SymfoniHabitTracker = {
            instance: instance,
            factory: _signer ? new HabitTracker__factory(_signer) : undefined,
        };
        return contract;
    };
    ...
    return (
        <SymfoniContext.Provider
            value={{ init: (provider) => handleInitProvider(provider), providers: providerPriority, currentHardhatProvider, loading, messages }}
        >
            <ProviderContext.Provider value={[provider, setProvider]}>
                <SignerContext.Provider value={[signer, setSigner]}>
                    <CurrentAddressContext.Provider value={[currentAddress, setCurrentAddress]}>
                        <HabitSourceContext.Provider value={HabitSource}>
